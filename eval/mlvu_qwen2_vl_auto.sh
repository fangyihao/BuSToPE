...
for MODEL_ENTRY in "${MODEL_LIST[@]}"; do
    ...
    for context_length in "${CONTEXT_LENGTHS[@]}"; do
        OUTPUT_FOLDER="results/mlvu/${CKPT}-${context_length}-${MIN_PIXELS_FACTOR}tokens"
        
        for IDX in $(seq 0 $((CHUNKS-1))); do
            CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 model_mlvu_qwen2_vl.py \
                --model-path ${CKPT_DIR}/${CKPT} \
                --max_new_tokens 128 \
                --Eval_Video_root /data/MVLU/MLVU/video \
                --Eval_QA_root /data/MVLU/MLVU/json \
        ...
        python3 check_mlvu_choices.py $OUTPUT_FOLDER
    done
done
